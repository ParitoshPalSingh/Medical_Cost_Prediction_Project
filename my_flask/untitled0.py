...
def Results():
    if request.method=="POST":
        ...
        try:
            ...
            db.session.add(pers_age)
            db.session.add(pers_sex)
            db.session.add(pers_bmi)
            db.session.add(pers_region)
            db.session.add(pers_children)
            db.session.add(pers_smoker)
...
def main():
    ...
    sql_create_guests_table = """CREATE TABLE IF NOT EXISTS friends(
    ID integer PRIMARY_KEY,
    Name String(50) NOT NULL,
    Age Integer NOT NULL,
    Sex String(6) NOT NULL, 
    Bmi Float NOT NULL,
    Region String(12) NOT NULL,
    Children Integer NOT NULL,
    Smoker String(4) NOT NULL,
    );"""
